...
var fullscreen = (metadata["fullscreen"]) ?? false;
...
var offscreen_handle = canvas.transferControlToOffscreen();
...
var dx = event_.offsetX - this.pointer_last_x;
var dy = event_.offsetY - this.pointer_last_y;
        this.pointer_last_x = event_.offsetX;
        this.pointer_last_y = event_.offsetY;
...
var wh = desktop_size[1];
  ...
  updateFocus() {
    if (this.focused) {
      ...
      // Update the icon
      if (this.icon !== null) {
var source = this.update_icon(this.icon.width, this.icon.height, this.icon.encoding, this.icon.img_data);
...
var title = Utilities.s(metadata["title"]);
      if (this.title != title) {
        this.title = title;
...
var attributes = ["modal", "above", "below"];
for(var attribute of attributes) {
      if (attribute in metadata) {
var value = metadata[attribute];
        if (value) {
...
var tclass = `${class_}`;
      ...
      if (wm_class) {
        //add new wm-class:
for(var element of wm_class) {
var tclass = Utilities.s(element).replace(/[^\dA-Za-z]/g, "");
...
var geom = this.get_internal_geometry();
...
var top = Math.round(e.position.top);
...
for(var screen_size of screen_sizes) {
        w = screen_size[0];
        h = screen_size[1];
...
var resize_widget = jQuery(this.div)
...
var window_element = jQuery(`#${this.wid}`);
var cursor_url = this.construct_base64_image_url(encoding, img_data);
var me = this;
    function set_cursor_url(url, x, y, w, h) {
var url_string = `url('${url}')`;
...
var bitmap = coding.startsWith("bitmap:");
    if (bitmap) {
...
var rgb_data = decode_rgb(packet);
var img = this.offscreen_canvas_ctx.createImageData(enc_width, enc_height);
...
for(var index = 0, stop = img_data.length; index < stop; ++index) {
var scroll_data = img_data[index];
          this.debug("draw", "scroll", index, ":", scroll_data);
var sx = scroll_data[0];
var sy = scroll_data[1];
var sw = scroll_data[2];
var sh = scroll_data[3];
var xdelta = scroll_data[4];
var ydelta = scroll_data[5];
...
var imageDataBase64 = Utilities.ArrayBufferToBase64(imageDataArrayBuffer);
