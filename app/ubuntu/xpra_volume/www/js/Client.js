...
var me = this;
var screen_element = jQuery("#screen");
...
var div = document.querySelector("#screen");
  ...
  send_log(level, arguments_) {
    if (this.remote_logging && this.server_remote_logging && this.connected) {
      try {
var sargs = [];
for(var argument of arguments_) {
...
var use_worker = with_worker && !XPRA_CLIENT_FORCE_NO_WORKER;
...
var uri = "";
  ...
  redraw_windows() {
for(var wid in this.id_to_window) {
var win = this.id_to_window[wid];
...
var packet_type = Utilities.s(packet[0]);
    this.debug("network", "received a", packet_type, "packet");
var function_ = this.packet_handlers[packet_type];
...
var packet = [PACKET_TYPES.desktop_size, newsize[0], newsize[1], this._get_screen_sizes()];
    this.send(packet);
    // call the screen_resized function on all open windows
for(var index in this.id_to_window) {
var win = this.id_to_window[index];
      win.screen_resized();
...
var preview_element = $(WINDOW_PREVIEW_SELECTOR);
...
var number_slides = $(".window-preview-item-container").length;
var current_slide = preview_element.slick("slickCurrentSlide");
...
var number_slides = slick.slideCount;
...
for(var key of keyboardLayoutMap.keys()) {
var value = keyboardLayoutMap[key];
...
var map_string = this.keyboard_map[keyname];
...
var is_clipboard_modifier_set = raw_modifiers.includes(clipboard_modifier);
...
var wid = this.topwindow;
var packet = [PACKET_TYPES.key_action, wid, keyname, pressed, modifiers, keyval, keystring, keycode, group];
...
for(var keycode in CHARCODE_TO_NAME) {
  ...
  _get_DPI() {
var dpi_div = document.querySelector("#dpi");
...
for(var attribute in appendobj) {
...
var digests = this._get_digests();
  ...
  _get_digests() {
var digests = ["hmac", "hmac+md5", "xor", "keycloak"];
...
for(var hash in forge.md.algorithms) {
  ...
  _get_cipher_caps() {
var enc = this.encryption.split("-")[0];
    if (enc != "AES") {
      throw `invalid encryption specified: '${enc}'`;
    }
var mode = this.encryption.split("-")[1] || "CBC";
    return {
      "cipher": enc,
      "mode": mode,
...
for(var target of [TEXT_HTML, UTF8_STRING, "TEXT", "STRING", TEXT_PLAIN]) {
...
for(var button of this.buttons_pressed) {
...
var lbe = this.last_button_event;
...
var button_x = this.wheel_delta_x >= 0 ? 6 : 7;
var button_y = this.wheel_delta_y >= 0 ? 5 : 4;
    while (wx >= 120) {
      wx -= 120;
...
var data = Utilities.StringToUint8(html);
...
var data = Utilities.StringToUint8(text);
            this.send_clipboard_token(data, [TEXT_PLAIN]);
          },
          (error) => this.cdebug("clipboard", "paste event failed:", error)
        );
      } else {
var text = clipboardData.getData(TEXT_HTML);
var datatype = TEXT_HTML;
        if (!text) {
...
var clipboard_buffer = this.get_clipboard_buffer();
var pasteboard = $(PASTEBOARD_SELECTOR);
...
var clipboard_buffer = this.get_clipboard_buffer();
var clipboard_datatype = (this.get_clipboard_datatype() || "").toLowerCase();
var is_text = clipboard_datatype.includes("text") || clipboard_datatype.includes("string");
    if (!is_text) {
...
for(var index in this.id_to_window) {
var otherwin = this.id_to_window[index];
...
var top_stacking_layer = Object.keys(this.id_to_window).length;
var old_stacking_layer = win.stacking_layer;
var had_focus = this.focus;
    this.focus = wid;
    this.topwindow = wid;
...
var wid = $(".slick-current .window-preview-item-container").data("wid");
var win = this.id_to_window[wid];
      if (!win.minimized) {
        win.focus();
      }
...
for(var index in windows_sorted) {
...
var code = Number.parseInt(packet[2]);
var reconnect = this.reconnect || this.reconnect_attempt < this.reconnect_count;
...
for(var CIPHER_CAP of CIPHER_CAPS) {
var cipher_key = `cipher${CIPHER_CAP}`;
var value = hello[cipher_key];
...
var version = Utilities.s(hello["version"]);
    try {
var vparts = version.split(".");
var vno = vparts.map((x) => Number.parseInt(x));
      if (vno[0] <= 0 && vno[1] < 10) {
...
for(var codec of MediaSourceConstants.PREFERRED_CODEC_ORDER) {
...
var command = entry.Exec.replace(/%[FUfu]/g, "");
...
var image_type = "image/"+icon_type;
      if (icon_type == "svg") {
...
var digest = packet[3];
var server_salt = Uint8ToString(packet[1]);
var salt_digest = packet[4] || "xor";
var prompt = (packet[5] || "password").replace(/[^\d+,. /:a-z]/gi, "");
...
var password = this.passwords.shift();
...
var address = `${client.host}:${client.port}`;
      this.cancel_hello_timer();
...
var salt = this._gendigest(salt_digest, client_salt, server_salt);
    if (!salt) {
      this.disconnect(`server requested an unsupported salt digest ${salt_digest}`);
      return;
    }
var challenge_digest = digest.startsWith("keycloak") ? "xor" : digest;
    this.clog("challenge using digest", challenge_digest);
var challenge_response = this._gendigest(challenge_digest, password, salt);
    if (challenge_response) {
...
var now_ms = Math.ceil(performance.now());
...
var event = document.createEvent("Event");
  ...
  position_float_menu() {
var float_menu_element = $(FLOAT_MENU_SELECTOR);
...
var float_tray = document.querySelector("#float_tray");
var float_menu = document.querySelector(FLOAT_MENU_SELECTOR);
var float_menu_element = $(FLOAT_MENU_SELECTOR);
...
var win = new XpraWindow(this, wid,
      x, y, w, h,
      metadata,
...
var trimmedTitle = Utilities.trimString(win.title, 30);
...
var geom = win.get_internal_geometry();
var wid = win.wid;
var packet = [PACKET_TYPES.configure_window, wid, geom.x, geom.y, geom.w, geom.h,
...
var button = packet[5];
var source_indication = packet[6];
...
var shadow_pointer = document.querySelector("#shadow_pointer");
...
var win = this.id_to_window[wid];
  ...
  auto_focus() {
var highest_window = null;
var highest_stacking = -1;
for(var index in this.id_to_window) {
var win = this.id_to_window[index];
...
var [, wid, x, y, width, height] = packet;
...
var summary = Utilities.s(packet[6]);
var body = Utilities.s(packet[7]);
...
var win = this.id_to_window[wid];
    if (win) {
var source = win.update_icon(w, h, encoding, img_data);
      //update favicon too:
...
var coding = Utilities.s(packet[6]);
...
var wid = packet[1];
  ...
  draw_pending_list() {
var elapsed = performance.now() - this.draw_pending;
...
var w = this.pending_redraw.shift();
      w.draw();
    }
  }
...
var packet = [PACKET_TYPES.damage_sequence, packet_sequence, wid, width, height, decode_time, message];
    if (decode_time < 0) {
...
var decode_time = Math.round(1000 * performance.now() - 1000 * start);
      if (flush == 0) {
...
var decode_time = options["decode_time"];
...
var codecs_supported = MediaSourceUtil.get_supported_codecs(this.audio_mediasource_enabled, this.audio_aurora_enabled, false);
...
var codec_string = MediaSourceConstants.CODEC_STRING[this.audio_codec];
...
var asb;
  ...
  _process_sound_data(packet) {
    try {
var codec = Utilities.s(packet[1]);
var buf = packet[2];
...
var size = ab.reduce(
          ...
          size = 0;
for(var index = 0, stop = ab.length; index < stop; ++index) {
var v = ab[index];
            if (v.length > 0) {
              buf.set(v, size);
              size += v.length;
...
var actual_data_format = data_format || [UTF8_STRING, TEXT_PLAIN];
...
var is_valid_target = target && this.clipboard_targets.includes(target);
    ...
    if (is_valid_target) {
var is_text = dtype.toLowerCase().includes("text") || dtype.toLowerCase().includes("string");
      if (is_text) {
        try {
...
for(var item_type of item.types) {
...
var primary_server_buffer = this.clipboard_server_buffers["PRIMARY"];
            if (
...
var packet = [PACKET_TYPES.clipboard_contents_none, request_id, selection];
...
var packet = [PACKET_TYPES.clipboard_contents, request_id, selection, datatype || UTF8_STRING, 8, "bytes", clipboard_buffer];
...
var packet = [PACKET_TYPES.clipboard_contents, request_id, selection, datatype, dformat || 8, encoding || "bytes", clipboard_buffer];
    this.send(packet);
  }
...
var basefilename = Utilities.s(packet[1]);
var mimetype = Utilities.s(packet[2]);
var printit = packet[3];
var filesize = packet[5];
var data = packet[6];
var options = packet[7] || {};
var send_id = Utilities.s(packet[8]);
...
var digest = null;
for(var hash_function of ["sha512", "sha384", "sha256", "sha224", "sha1"]) {
...
var chunk_id = Utilities.s(options["file-chunk-id"] || "");
    if (!chunk_id) {
...
var fileStream = streamSaver.createWriteStream(basefilename, {
        size: filesize,
      });
...
var openit = true;
var chunk_state = [Date.now(), writer, basefilename, mimetype, printit, openit, filesize,
...
var chunk_state = this.receive_chunks_in_progress.get(chunk_id);
...
for(var chunk_id of this.receive_chunks_in_progress.keys()) {
...
var chunk_state = this.receive_chunks_in_progress.get(chunk_id);
    if (chunk_state) {
      ...
      chunk_state[10] = true;
      //free the buffers
var writer = chunk_state[1];
      if (writer.abort) {
        writer.abort();
      }
      chunk_state[1] = null;
      //stop the timer
var timer = chunk_state[12];
      if (timer) {
        clearTimeout(timer);
        chunk_state[12] = 0;
      }
...
var chunk_id = Utilities.s(packet[1]);
var chunk = packet[2];
var file_data = packet[3];
var has_more = packet[4];
...
var written = chunk_state[9] + file_data.length;
    if (written > filesize) {
...
var p = writer.write(file_data);
        ...
        if (p) {
          p.then(
            ...
            (error) => {
var message = "cannot write file data, download cancelled?";
              if (error) {
...
var writer = chunk_state[1];
var filesize = chunk_state[6];
var digest = chunk_state[8];
var written = chunk_state[9];
    if (digest) {
...
var chunks = chunk_state[1];
for(var chunk_ of chunks) {
...
var hex_value = Utilities.convertToHex(value);
...
var u8a = new Uint8Array(event_.target.result);
...
var cdata = buffer;
var options = {};
var chunk_size = Math.min(FILE_CHUNKS_SIZE, this.remote_file_chunks || 0);
...
var chunk_id = Utilities.getHexUUID();
...
var chunk_state = [Date.now(), buffer, chunk_size, timer, 0];
...
var packet = [PACKET_TYPES.send_file, filename, mimetype, false, this.remote_open_files, size, cdata, options];
    this.send(packet);
  }

  _check_chunk_sending(chunk_id, chunk_no) {
var chunk_state = this.send_chunks_in_progress.get(chunk_id);
...
var start_time = chunk_state[0];
var chunk_size = chunk_state[2];
var timer = chunk_state[3];
var data = chunk_state[1];
    if (!data) {
      //all sent!
var elapsed = Date.now() - start_time;
var bps = (8 * chunk * chunk_size) / elapsed;
...
var cdata = data.subarray(0, chunk_size);
...
var packet = [PACKET_TYPES.start_command, name, command, ignore];
    this.send(packet);
  }
...
var body = `<a href="${url}" rel="noopener" target="_blank">${url}</a>`;
var timeout = 10;
