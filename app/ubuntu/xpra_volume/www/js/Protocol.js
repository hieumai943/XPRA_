...
var n = Math.min(needed, slice.length);
      ...
      if (this.header[0] !== 80) {
var message = `invalid packet header format: ${this.header[0]}`;
        if (this.header.length > 1) {
var hex = "";
for(var p of this.header) {
var v = p.toString(16);
...
var packet_size = [4, 5, 6, 7].reduce(
...
var input_data =
        ...
        if (padding_size) {
var padding_char = String.fromCharCode(padding_size);
...
var packet_data = new Uint8Array(actual_size + 8);
var level = 0;
      //header:
...
for(var index = 0; index < actual_size; index++) {
...
var mode = caps["cipher.mode"] || DEFAULT_MODE;
var block_size = 0;
    if (mode == "CBC") {
      block_size = 32;
